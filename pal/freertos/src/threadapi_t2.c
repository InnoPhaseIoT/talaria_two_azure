...
THREADAPI_RESULT ThreadAPI_Create(THREAD_HANDLE* threadHandle, THREAD_START_FUNC func, void* arg)
{
    ...
    else
    {
        ...
        *threadHandle = (void *)xTaskCreate((void *)func, /* The function that implements the task. */
            "az_pal", /* The text name assigned to the task - for debug only
                            * as it is not used by the kernel. */
            AZ_PAL_THREAD_STACK_SIZE, /* The size of the stack to allocate to the task. */
            arg, /* The parameter passed to the task - not used in this case. */
            AZ_PAL_THREAD_PRIO, /* The priority assigned to the task. */
            NULL);
