...
#define CONFIG_MESSAGE_INTERVAL_TIME 2000
...
static IOTHUBMESSAGE_DISPOSITION_RESULT ReceiveMessageCallback(IOTHUB_MESSAGE_HANDLE message, void* userContextCallback)
{
    ...
    {
        os_printf("unable to retrieve the message data\r\n");
    }
    else
    {
        os_printf("Received Message [%d]\r\n Message ID: %s\r\n Correlation ID: %s\r\n Data: <<<%.*s>>> & Size=%d\r\n", *counter, messageId, correlationId, (int)size, buffer, (int)size);
...
void iothub_client_sample_mqtt_run(void)
{
    ...
    if (platform_init() != 0)
    {
        os_printf("Failed to initialize the platform.\r\n");
    }
    else
    {
        if ((iotHubClientHandle = IoTHubClient_LL_CreateFromConnectionString(connectionString, MQTT_Protocol)) == NULL)
        {
            os_printf("ERROR: iotHubClientHandle is NULL!\r\n");
        }
        else
        {
            bool traceOn = true;
            ...
            {
                os_printf("ERROR: IoTHubClient_LL_SetMessageCallback..........FAILED!\r\n");
            }
            else
            {
                os_printf("IoTHubClient_LL_SetMessageCallback...successful.\r\n");
                ...
                do
                {
                    //os_printf("iterator: [%d], callbackCounter: [%d]. \r\n", iterator, callbackCounter);
                    time(&current_time);
                        ...
                        && (difftime(current_time, sent_time) > ((CONFIG_MESSAGE_INTERVAL_TIME) / 1000)))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                os_printf("ERROR: Map_AddOrUpdate Failed!\r\n");
                            }
                            ...
                            {
                                time(&sent_time);
                                os_printf("IoTHubClient_LL_SendEventAsync accepted message [%d] for transmission to IoT Hub.\r\n", (int)iterator);
                            }
                        }
                        iterator++;
                    }
                ...
                os_printf("iothub_client_sample_mqtt has gotten quit message, call DoWork %d more time to complete final sending...\r\n", DOWORK_LOOP_NUM);
                size_t index = 0;
