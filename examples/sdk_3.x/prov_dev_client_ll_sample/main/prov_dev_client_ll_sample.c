...
static IOTHUBMESSAGE_DISPOSITION_RESULT receive_msg_callback(IOTHUB_MESSAGE_HANDLE message, void* user_context)
{
    ...
    os_printf("Stop message recieved from IoTHub\r\n");
...
static void iothub_connection_status(IOTHUB_CLIENT_CONNECTION_STATUS result, IOTHUB_CLIENT_CONNECTION_STATUS_REASON reason, void* user_context)
{
    ...
    {
        os_printf("iothub_connection_status user_context is NULL\r\n");
    }
    else
    {
        IOTHUB_CLIENT_SAMPLE_INFO* iothub_info = (IOTHUB_CLIENT_SAMPLE_INFO*)user_context;
...
{
    os_printf(" register_device_callback iothub_uri: %s! device_id: %s!\r\n", iothub_uri, device_id);
    if (user_context == NULL)
    {
        os_printf("user_context is NULL\r\n");
    }
    else
    {
        ...
        {
            os_printf("Registration Information received from service: %s!\r\n", iothub_uri);
...
int prov_dev_client_ll_sample_run()
{
...
#if defined(HSM_TYPE_X509)
    hsm_type = SECURE_DEVICE_TYPE_X509;
#endif

#if defined(HSM_TYPE_SAS_TOKEN)
    hsm_type = SECURE_DEVICE_TYPE_TPM;
#endif


    /* Change this to false to disable verbose trace logs */
    bool traceOn = true;
    ...
    /* Note --
    calling this here overrides the 'SYMMETRIC_KEY' and 'REGISTRATION_NAME', which are the init values set in function custom_hsm_create() 
    in file custom_hsm.c. (These values are defined  on top of custom_hsm.c file). */
#endif
    ...
    os_printf("Provisioning API Version: %s\r\n", Prov_Device_LL_GetVersionString());
    os_printf("Iothub API Version: %s\r\n", IoTHubClient_GetVersionString());
    ...
    {
        os_printf("failed calling Prov_Device_LL_Create\r\n");
    }
    else
    {
        ...
        {
            os_printf("failed calling Prov_Device_LL_Register_Device\r\n");
        }
        else
        {
            do
    ...
    else
    {
        ...
        os_printf("Creating IoTHub Device handle\r\n");
        ...
        {
            os_printf("failed create IoTHub client from connection string %s!\r\n", user_ctx.iothub_uri);
        }
        else
        {
            ...
            os_printf("Sending 1 messages to IoTHub every %d seconds for %d messages (Send any message to stop)\r\n", TIME_BETWEEN_MESSAGES, MESSAGES_TO_SEND);
            do
            {
                if (iothub_info.connected != 0)
                {
                    ...
                    if ((current_tick - last_send_time) / 1000 > TIME_BETWEEN_MESSAGES)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                os_printf("ERROR: IoTHubClient_LL_SendEventAsync..........FAILED!\r\n");
                            }
                            else
                            {
                                (void)tickcounter_get_current_ms(tick_counter_handle, &last_send_time);
                                os_printf("IoTHubClient_LL_SendEventAsync accepted message [%zu] for transmission to IoT Hub.\r\n", msg_count);
